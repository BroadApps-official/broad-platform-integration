...
Для существующего приложения доступен [единый reviewer](../Documentation/AgentReview.md):
`app PATH --scheme NAME` проверяет фактическую интеграцию без начала workflow
создания приложения. Отчёт хранится в app `.build/AgentReview/app/`.
...
| RU Billing | Current-provider `ru_pay`, backend kill switch, catalog/auth/legal | `BLOCKED`, если нет explicit gate или он заменён fixture |
