...
`f45eb08203bad0600a52c086df3e05c8dabd597fae7041539217b57f10500471`.
...
`PASS` — 15 августа 2026 года. README определяет ровно два типа проекта — с
Figma и no-code без Figma, отдельно объясняет самостоятельный поиск reference и
разделяет агента разработки приложения с проверяющим агентом платформы. Для
Codex/Claude добавлены готовые copy-paste prompts, а для ручной работы — точный
порядок сборки и запуска gate. Условия recovery после переустановки,
server-authoritative token balance и защиты финансовых flow при внезапном
обрыве сети сохранены. Полный `agent_gate` прошёл для указанного snapshot.
