...
При каждой сборке Xcode и platform gate проверяется
[`Configuration/AccountIntegration.json`](Configuration/AccountIntegration.json).
Предупреждения укажут на demo-баланс, ненастроенное восстановление аккаунта и
невыбранную политику iCloud. Если токенов и баланса в приложении нет, разработчик
или агент ставит `tokens.mode = notUsed` и пишет причину в `details` — backend
для этой функции не требуется. Аккаунт и iCloud выбираются отдельно.
Не удаляйте build phase при копировании шаблона. [Все значения и правила](../../Documentation/AccountRecovery.md#предупреждения-при-сборке-шаблона).
