...
Если выдан старый reference-проект, из него берутся нужные экраны, поведение,
тексты и app-owned данные, но не случайная архитектура. Если есть Figma — дизайн
берётся из неё. Если Figma нет — сначала используется согласованный результат
Claude Design или Pencil; UI не придумывается вручную в SwiftUI «на глаз».
