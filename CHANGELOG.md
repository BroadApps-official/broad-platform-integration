...
- README теперь начинает работу с реального сценария команды: новое приложение
  строится поверх платформы; тип определяется только наличием Figma;
  reference разработчик сначала ищет в Kaiten/Git компании, а при необходимости
  запрашивает у Team Lead или ПМ; отдельные copy-paste инструкции объясняют
