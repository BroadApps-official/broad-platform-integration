...
| Серверная отмена checkout (опционально для account-policy с 5.0.0) | Если поддерживается: method/path/auth, корреляция попытки и terminal response; иначе notUsed |  |  |
...
| Как завершается ожидание checkout? | Account-policy с BroadMonetization 5.0.0: `waitingCompleted`/`awaitingReconciliation`, fresh policy перед новой покупкой; paymentStatus: terminal backend result. `checkoutTerminationClient` опционален для явной серверной отмены |  |
