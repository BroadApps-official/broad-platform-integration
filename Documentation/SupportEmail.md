...
### Дополнительные данные текущего аккаунта

С BroadUIFlows 4.1.0 после `Subscription`, перед пустой строкой и
`--- Diagnostics ---`, добавляются доступные данные:

```text
Device ID: <DEVICE_ID>
Token balance: <CONFIRMED_TOKEN_BALANCE>
Token account ID: <ACCOUNT_ID_USED_FOR_TOKEN_CREDITS>
```

Это пример дополнительных строк, а не обязательные поля каждого приложения.
Передавайте все доступные ID текущего аккаунта, особенно тот, по которому backend
начисляет токены. Device ID и ID токенного аккаунта могут различаться.
`additionalIdentifiers: [BroadSupportEmailIdentifier]` позволяет передать другие
ID с понятными стабильными названиями в согласованном порядке. Не дублируйте уже
переданные Adapty/backend/device ID.

Баланс добавляется только в приложениях с токенами, если подтверждённое значение
известно. Неизвестный баланс пропускается, настоящий ноль сохраняется.
Пустые дополнительные поля пропускаются; переводы строк внутри них заменяются
пробелами. Обычный и RU-шаблоны используют один и тот же набор дополнительных полей.
Первая строка, базовые поля, секции и их порядок остаются прежними.
...
| Дополнительные ID и подтверждённый баланс после Subscription по правилам выше | Базовые поля и порядок секций |
...
Используй optional поля BroadUIFlows 4.1.0; неизвестный баланс не заменяй нулём.
Прикрепи очищенный support log. Не
