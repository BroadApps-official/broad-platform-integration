...
| 15 | `BroadLogEvent` + `OSLogBroadLogger` + `RUBillingDebugOverrideStore` | Закрытые enum/Bool/count; `ru_pay` только из current provider payload в Release, tri-state только Debug |
...
| `ru_pay` provider true/false | Сравнить `-ru-pay-provider-enabled` и `-ru-pay-provider-disabled` | Provider enabled показывает Apple/СБП/карту при RU-контексте; disabled — только Apple | `ru-billing.availability.evaluated` с typed reason и method count | Ничего | Принимать absent/false/invalid flag |
| Platform cache rejection | Запустить `-ru-pay-platform-cache` | Даже сохранённый `ru_pay = true` оставляет только Apple | Typed reason без raw config/path | Ничего | Считать persistent cache provider payload |
...
| Старые схемы показывали один build prompt и второй prompt проверки | Просмотреть roadmap, agent click path и обе no-code схемы | Все схемы показывают Integration Plan, один slice за раз, checkpoints и acceptance; старых двух prompts/PASS нет | Шаг 13 |
...
| После `BLOCKED` было непонятно, начинать ли заново | Снять blocker и продолжить в том же или новом чате | Читаются Plan/checkpoint/diff; повторяется только остановленный stage, принятые slices не генерируются заново | Шаги 11–13 |
