...
| Paywall закрыт, special offer разрешён | Закрыть initial paywall в enabled fixture | Special offer → close/purchase → main | `[FLOW] initial-paywall → special-offer` | Offer eligibility/cooldown только в своём store | Зацикливать offer или доверять feature gate из platform cache |
...
| Analytics | Открыть/закрыть safe paywall и открыть аналитику | Счётчик, live-список, время update и понятное empty state | `[ANALYTICS] analytics.events.recorded count=N` | Bounded in-memory history только до закрытия process | Показывать секреты/raw payload или обещать persistence |
...
| Kaiten и дизайн доступны | Выполнить preflight-промпт | Отчёт `Kaiten/design/reference/backend` и `Можно начинать: ДА` | `[INPUT]` остаётся в видимом отчёте агента; runtime Console здесь не участвует | Ничего | Начинать код до отчёта |
...
| В Variant A было непонятно, где взять основной prompt | Нажать ссылку сразу после схемы Variant A | README прокручивается точно к полному build prompt | Шаг 13 |
| Было непонятно, где взять prompt проверки | Нажать ссылку рядом с build prompt | README прокручивается точно к app-check prompt | Шаг 13 |
...
| API-логику пришлось дописывать после основного prompt | Сверить каждую функцию с backend matrix | До UI зафиксированы method/endpoint/request/response/auth/errors/retry и все расхождения | Шаг 11 |
