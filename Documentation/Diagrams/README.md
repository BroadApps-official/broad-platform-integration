...
В кратком README показаны баннер перехода на сайт и карта выбора модулей.
Остальные материалы используются в подробных инструкциях; их наличие здесь
не означает, что все они должны снова попасть в README.

`ruby Scripts/generate_federation_diagrams.rb` воспроизводит оба баннера
`hero-*.svg` и схемы repositories. Баннер содержит адрес актуального сайта,
а карта модулей показывает независимый выбор products, без ложной цепочки
Extensions → Core → Monetization → UIFlows.
