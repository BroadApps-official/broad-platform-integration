...
Настройка продуктов, точного приложения и A/B-теста в кабинете не является
задачей iOS-разработчика. Пошаговая публичная инструкция для аккаунт-менеджера:
[RU Billing для аккаунт-менеджера](https://broadapps-ios-docs.nkhsnv.chatgpt.site/docs/ru-billing-account-manager).

Разработчик должен получить от менеджера заполненную карточку входных данных и
не копировать значения из другого приложения.
