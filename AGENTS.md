...
- С BroadMonetization 2.0.1 Remote Config читается из paywall текущего
  placement; main заполняет только отсутствующие ключи. False/invalid/null
...
- Региональное условие RU Billing: текущий App Store Storefront `RU/RUS` **или**
  ...
  авторизует оплату.
- Полный backend-каталог сохраняется в исходном порядке, включая дубли.
...
- Для обычного RU-экрана при отсутствии всех точных ID (или продуктов Adapty)
  выбираются все подписки с `isDefault=true`, без строк Special Offer. Без
