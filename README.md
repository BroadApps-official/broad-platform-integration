...
2. Backend-матрицу для каждой функции: HTTP method, endpoint, request, response,
   ...
   доказанного контракта не считается подключённой.
...
14. Сначала проверь безопасные локальные данные, затем только загрузку настоящего
    каталога Adapty без финансовых операций.
15. Собери приложение в Debug и Release. Запусти безопасные сценарии. Ни одна
...
**Итерация 2 — визуальная.** Для каждого экрана агент открывает исходный
Figma/no-code frame, запускает то же состояние на таком же iPhone Simulator,
делает screenshot и сравнивает композицию, шрифты, цвета, отступы, assets и
состояния. Расхождения исправляются, после чего сравнение повторяется. Только
после этой итерации разработчик сам просматривает приложение и передаёт его QA.
...
4. Без настоящих платежей проверь безопасные сценарии: первый запуск,
   onboarding с фактическим количеством страниц проекта, paywall, 0/1/2/12
   продуктов, ошибку/повтор, незавершённую операцию, отсутствие сети и основной
   экран после подтверждения доступа.
...
14. Раздели проверку на функциональную и визуальную. Сначала докажи маршруты,
    API, onboarding, subscription/token paywall, token callback, loader,
    error/offline и Debug/Release. Затем на одном размере iPhone сравни каждый
    экран с его Figma/no-code frame по screenshot: композицию, шрифты, цвета,
    отступы, assets и состояния; исправь расхождения и повтори сравнение. Не пиши
    PASS и не предлагай передавать приложение QA до обеих итераций и личного
    просмотра разработчиком.
...
> доступа, а временные конфиги собраны в одном месте и
