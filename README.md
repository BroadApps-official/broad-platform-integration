...
| `BroadMonetization` | Adapty, StoreKit, placements, experiments, purchase/restore, entitlement, tokens и RU Billing |
| `BroadUIFlows` | App flow, onboarding, paywall, loader/error/retry и общий RU Billing UI |
| `BroadExtensions` | Независимые helpers для Hex Color, fonts, keyboard и scoped swipe-back |
| `BroadAppTemplate` | Запускаемый пример сборки зависимостей и безопасных fixture-сценариев |
...
> приложение. Отдельный проверяющий агент из задания Никиты нужен только тогда,
> когда кто-то изменил исходники самой `BroadAppsIOSPlatform`; его запуск описан
> в шаге 6 и в разделе [«Как проверять платформу»](#automation).

### 1. Покажите агенту рабочую папку и исходные материалы

Агент должен видеть:

1. пустую или подготовленную папку, где создаётся **новое приложение**;
2. этот repository локально или его SPM URL;
3. документ текущего проекта в Kaiten;
4. ссылку на Figma либо явную пометку `NO-CODE` и согласованный результат Claude
   Design/Pencil;
5. reference, если разработчик уже нашёл его в Git компании.
...
3. Определи тип проекта только по Figma: ссылка есть — проект с Figma; ссылки нет — no-code и нужен согласованный дизайн из Claude Design/Pencil.
4. Если в строке Reference написано НАЙДИ, сначала проверь документ Kaiten и доступные Git-репозитории/live-приложения компании. Если нельзя однозначно выбрать похожий проект, остановись и попроси меня запросить reference у Team Lead или проектного менеджера.
5. Reference используй только для понимания экранов, сценариев, контента и временных рабочих конфигов. Не изменяй его и не копируй из него архитектуру.
6. Если данных текущего приложения пока нет, возьми временную development-конфигурацию найденного похожего live-приложения. Пометь все такие значения как временные и сохрани их в app-owned configuration для простой замены перед релизом.
7. Создай новый iPhone-проект на модулях платформы. Названия scheme/target возьми из названия проекта; остальные доступные значения — из Kaiten.
8. Используй BroadAppTemplate как пример composition и fixtures.
...
Разработчику нужно заполнить только четыре короткие строки сверху: название,
документ Kaiten, `Figma/NO-CODE` и reference. Если reference пока не найден,
достаточно написать `НАЙДИ` — агент сначала проверит доступные источники и не
будет заставлять разработчика вручную перепечатывать конфигурацию.
...
### 6. Скопируйте агенту промпт проверки приложения

После реализации отправьте **в тот же чат Codex или Claude** этот текст:

```text
Проверь созданное iPhone-приложение и исправь найденные проблемы.

1. Не изменяй reference-проекты.
2. Проверь текущий diff и соответствие выбранному источнику дизайна: Figma либо NO-CODE.
3. Собери приложение для iPhone Simulator в Debug и Release.
4. Без реальных платежей проверь доступные fixture-сценарии: first run, onboarding, paywall, 0/1/2/12 продуктов, error/retry, pending, offline и verified main.
5. Проверь, что ATT вызывается только после появления первого onboarding-слайда, Rate Us отсутствует в onboarding, продукты Adapty не фильтруются, main используется как fallback, а purchase/restore/pending не открывают premium до fresh entitlement.
6. Если исходники BroadAppsIOSPlatform не изменялись, не запускай её исправляющего агента — просто напиши это в отчёте.
7. Если исходники BroadAppsIOSPlatform менялись, открой её корень, прочитай AGENTS.md и запусти bash Scripts/agent_gate.sh. Исправляй только platform-owned файлы и повторяй gate до PASS.

В конце напиши простым русским языком: что проверил, что нашёл, что исправил, какие файлы изменил, какие команды прошли и что осталось.
```

Это проверяет **конкретное приложение**. Проверяющий агент из задания Никиты
нужен для другого случая — когда разработчик изменил код самой платформы:
...
> [!WARNING]
> Не запускайте `agent_review_and_fix.sh` из уже работающего Codex/Claude: wrapper
> попытается создать второго агента. Внутри чата используется только
...
### Шаг 8. Проверьте, меняли ли вы саму платформу

Если вы только подключили package и писали код нового приложения, проверка
заканчивается шагом 7. Скрипты платформы не проверяют чужой app target.
...
|---|---|
| Только код нового приложения | Соберите app target в Debug/Release и выполните его safe fixture-сценарии. Platform agent не нужен |
...
|---|---|---|
| `./Scripts/agent_review_and_fix.sh` | Из Terminal, когда нужен автоматический поиск и исправление ошибок | Wrapper запускает Codex с готовым prompt, разрешает исправления только внутри платформы, повторно проверяет результат и сохраняет отчёт |
| `bash Scripts/agent_gate.sh` | Внутри уже открытого Codex/Claude либо вручную без автоматических исправлений | Запускает все правила, style checks и сборки; сам код не меняет |
...
| [`AgentChecks/AUTOMATION_PROMPT.md`](AgentChecks/AUTOMATION_PROMPT.md) | Готовое задание, которое wrapper передаёт Codex |
