...
1. Изучи Kaiten, Figma/no-code материалы, техническое задание и reference.
2. Выпиши все найденные слайды по порядку.
3. Если количество или содержимое неоднозначно — сначала задай мне вопрос.
4. Не используй три демонстрационных слайда BroadAppTemplate как значение по умолчанию.
5. Количество страниц задавай только массивом OnboardingConfiguration.pages.
6. Если нужен уникальный дизайн, используй BroadOnboardingFlowHost и создай интерфейс приложения отдельно.
7. ATT разрешён только после фактического появления первого слайда; при отключённом onboarding ATT не вызывай.
...
> происходит на последнем элементе `pages`, ведёт на paywall, а основной экран
> невозможно открыть без подтверждённого доступа.
