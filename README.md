...
|---|---|---|
| [**1. Проверить доступ к материалам →**](#agent-preflight) | [**2. Отправлять staged prompts по одному →**](#agent-staged-prompts) | [**3. Скопировать prompt финальной проверки →**](#agent-app-check) |
...
**Что делает разработчик лично:** открывает рабочую папку, даёт агенту исходные
ссылки, а затем подтверждает `PLAN`, `SKELETON`, `FUNCTIONAL` и `VISUAL`
checkpoints. Он принимает app-owned решения, но не переписывает platform logic и
не проверяет каждый созданный файл построчно.
...
[`Documentation/Templates/AppIntegrationPlan.md`](Documentation/Templates/AppIntegrationPlan.md)
в repository приложения как `Documentation/AppIntegrationPlan.md`. Заполните
screen map, backend contracts, ownership и blockers. Даже без агента действует
тот же [поэтапный workflow](Documentation/AppCreationWorkflow.md): сначала план,
затем каркас, после него — один вертикальный срез за раз.
...
Ручной порядок тот же: сначала Integration Plan и functional checkpoints,
затем функциональная итерация (маршруты, API,
