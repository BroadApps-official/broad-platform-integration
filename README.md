...
запросить ближайший reference у тимлида-разработчика или проектного менеджера.
...
- работу backend: какие API-ручки вызываются, какие данные они принимают и
  возвращают, как обрабатываются авторизация, ошибки и повтор запросов.
...
| Результат сравнения | Что делать |
|---|---|
| Для функции есть подходящая ручка и её контракт достаточен | Использовать этот backend-контракт в новом приложении |
| Ручка есть, но не возвращает нужные данные или не поддерживает нужное действие | Остановиться и уточнить объём до реализации |
| Для новой функции ручки нет | Не придумывать API и не убирать функцию молча |

При любом несовпадении задайте тимлиду-разработчику или проектному менеджеру
три прямых вопроса:

1. Эта новая функция действительно согласована и должна войти в приложение?
...
До ответа не создавайте выдуманные endpoints, временные фиктивные ответы и не
сокращайте согласованный функционал самостоятельно.
...
Разработчику не нужно угадывать, «что брать». Он подключает нужные модули
Swift Package и использует их открытые интерфейсы:
...
Агент сначала проверяет, откуда брать дизайн: из Figma или согласованного
no-code результата. Затем он создаёт новый iPhone-проект на модулях платформы.
...
- Сначала разработчик сам ищет похожее live-приложение в документе проекта и
  ...
  запрашивается у тимлида-разработчика или проектного менеджера.
...
**✅ Шаг закончен, если:** вы понимаете, какие paywall/placement обязательны,
...
3. Проверь источник дизайна: ссылка на Figma есть — следуй ей; ссылки нет —
   используй согласованный no-code дизайн из Claude Design/Pencil.
...
**✅ Шаг закончен, если:** агент прочитал исходные материалы, начал работу в папке
нового приложения и не просит повторно перепечатать данные, которые уже есть в
Kaiten или reference.
...
**✅ Шаг закончен, если:** в ответе есть путь к проекту, список подключённых
...
Этот путь нужен, если агент не используется. Сначала проверьте источник
дизайна: Figma есть — работаете по Figma; Figma нет — используете согласованный
no-code дизайн из Claude Design/Pencil. Затем найдите reference в Kaiten или Git
...
зафиксированный вопрос тимлиду-разработчику/ПМ, а спорная разработка ещё не
начата.
...
настоящие платежи не запускались, а неопределённый результат не открыл основной
экран.
...
2. **Показываем все продукты Adapty в исходном порядке.** Не фильтруем, не сортируем и не объединяем одинаковые SKU — идентификаторы товаров в каталоге.
3. **Успешный ответ покупки/восстановления ещё не означает premium.** Основной экран открывается только после новой проверки доступа через StoreKit или сервер.
...
6. **Платёжные кнопки не мерцают.** При нажатии карточка не меняет прозрачность, не уменьшается и не затемняется; состояние обработки показывается отдельно.
